...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let userInformation = UserInformation(name: "Guillaume", surname: "Maiano", birthdate: {
            let dateStringFormatter = DateFormatter()
             dateStringFormatter.dateFormat = "yyyy-MM-dd"
            dateStringFormatter.locale = Locale(identifier: "en_US_POSIX")
            return dateStringFormatter.date(from: "1982-04-20")!
        }(), role: "Ingénieur Développement iOS",        address: UserCVAddress(city: "Lille",
                                   zipcode: "59800",
        ...
        let contentView = CVMainView(userInformation: userInformation)
